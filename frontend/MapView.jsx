...
export default function MapView({ 
  ...
  useEffect(() => {
    const fetchBoundaryEdgeLines = async () => {
      try {
        const response = await axios.get("http://127.0.0.1:5000/boundary-edge-lines");
  ...
  useEffect(() => {
    if (externalSelectedLine) {
      const newSelectedLine = {
        ...
        name: `${externalSelectedLine.lineType === 'INLINE' ? 'IL' : 'XL'}${externalSelectedLine.lineNumber}`
      };
  ...
  useEffect(() => {
    if (seismicSectionData && seismicSectionData.lineNumber && seismicSectionData.lineType) {
      const externalLine = {
        type: seismicSectionData.lineType,
        number: seismicSectionData.lineNumber.toString(),
        name: `${seismicSectionData.lineType === 'INLINE' ? 'IL' : 'XL'}${seismicSectionData.lineNumber}`
      };
  ...
  const filterLinesByDensity = useCallback((lines, lineType) => {
  ...
  const prepareTraces = useCallback(() => {
    ...
    if (viewMode === "both" || viewMode === "inlines") {
      filteredInlines.forEach((il) => {
        const isSelected = selectedLine && selectedLine.type === 'INLINE' && parseInt(selectedLine.number) === il.inline;
        const isHovered = hoveredLine && hoveredLine.type === 'INLINE' && parseInt(hoveredLine.number) === il.inline;
    ...
    if (viewMode === "both" || viewMode === "xlines") {
      filteredXlines.forEach((xl) => {
        const isSelected = selectedLine && selectedLine.type === 'XLINE' && parseInt(selectedLine.number) === xl.xline;
        const isHovered = hoveredLine && hoveredLine.type === 'XLINE' && parseInt(hoveredLine.number) === xl.xline;
    ...
    return traces;
  }, [gridData, boundary, viewMode, selectedLine, hoveredLine, lineDensity, filterLinesByDensity]);
  ...
  const getCurrentRanges = () => {
    if (currentLayout) {
      if (currentLayout['xaxis.range[0]'] !== undefined && currentLayout['xaxis.range[1]'] !== undefined) {
        return {
          xrange: [currentLayout['xaxis.range[0]'], currentLayout['xaxis.range[1]']],
          yrange: [currentLayout['yaxis.range[0]'], currentLayout['yaxis.range[1]']]
        };
  ...
  const getBoundaryEdgeLabels = useMemo(() => {
    if (!boundaryEdgeLines || !boundary || boundary.length < 4 || !gridData.inlines.length) {
        return [];
    }
    ...
    for (let edgeIdx = 0; edgeIdx < 4; edgeIdx++) {
        ...
        const displayedInlinesOnEdge = allInlinesOnEdge.filter(lineNum => lineNum % lineDensity === 0);
        ...
        displayedInlinesOnEdge.forEach((lineNum) => {
            ...
            const isAtCorner = boundary.slice(0, 4).some(
                corner => Math.hypot(corner[0] - intersectionPoint[0], corner[1] - intersectionPoint[1]) < 1.0
            );
            ...
            if (!isAtCorner) {
                ...
                edgeLabels.push({
                    x, y, text: `IL ${lineNum}`,
                    font: { color: "#00FFFF", size: 10, family: "Arial", weight: "bold" },
                });
            }
        });

        const allXlinesOnEdge = boundaryEdgeLines[edgeIdx]?.xlines || [];
        const displayedXlinesOnEdge = allXlinesOnEdge.filter(lineNum => lineNum % lineDensity === 0);
        ...
        displayedXlinesOnEdge.forEach((lineNum) => {
            ...
            if (!isAtCorner) {
                ...
                edgeLabels.push({
                    x, y, text: `XL ${lineNum}`,
                    font: { color: "#FF69B4", size: 10, family: "Arial", weight: "bold" },
                });
            }
        });
    }
  ...
  const getCoordinateAnnotations = () => {
    const { xrange, yrange } = currentRanges;
    const xSpan = xrange[1] - xrange[0];
    const ySpan = yrange[1] - yrange[0];
    const numTicks = Math.max(3, Math.min(8, Math.floor(Math.max(xSpan, ySpan) / 100)));
    ...
    xTicks.forEach(val => {
      if (val >= xrange[0] && val <= xrange[1]) {
        annotations.push({
          x: val, y: yrange[1],
          text: val.toLocaleString(), showarrow: false,
          font: { color: "white", size: 11, family: "Arial" },
          yshift: 25, xanchor: "center",
          bgcolor: "rgba(0,0,0,0.8)",
          bordercolor: "rgba(255,255,255,0.5)", borderwidth: 1, borderpad: 4
        });
      }
    });
    ...
    annotations.push(
      {
        ...
        y: yrange[1],
        text: "X Coordinate", showarrow: false,
        font: { color: "white", size: 12, family: "Arial", weight: "bold" },
        yshift: 45, xanchor: "center",
        bgcolor: "rgba(0,0,0,0.9)", bordercolor: "white", borderwidth: 1
      },
      {
        x: (xrange[0] + xrange[1]) / 2,
        y: yrange[0],
      ...
      },
      {
        ...
        text: "Y Coordinate", showarrow: false,
        font: { color: "white", size: 12, family: "Arial", weight: "bold" },
        xshift: -60, yanchor: "middle", textangle: -90,
        bgcolor: "rgba(0,0,0,0.9)", bordercolor: "white", borderwidth: 1
      },
      {
        x: xrange[1],
  ...
  const coordinateAnnotations = getCoordinateAnnotations();
  ...
  return (
    <div>
      <div style={{ 
        ...
        {selectedLine && (
          <div style={{ 
            ...
            <div>
              <strong>Selected: {selectedLine.type} {selectedLine.number}</strong>
            </div>
            <button 
      ...
      <Plot
        ...
        layout={{
          title: {
            text: `Seismic Survey Map - ${viewMode === 'both' ? 'Inlines & Crosslines' : viewMode === 'inlines' ? 'Inlines Only' : 'Crosslines Only'} (Showing every 50th line)`,
